...
def freeze_graph(model_dir, output_node_names):
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        images_placeholder = tf.placeholder(tf.float32,
                                            shape=(None, 32, 32, 3), name="Images_Placeholder")

        labels_placeholder = tf.placeholder(tf.int64, shape=(48, 10), name="Labels_Placeholder")

        network_fn = tt_conv_restore(images_placeholder, 10, {}, {}, {}, {}, {}, {}, {})
        # define activation of last layer as score
        score = network_fn.fc7

        # create op to calculate softmax
        softmax = tf.nn.softmax(score)
        labels = tf.argmax(softmax)

        # right_count_top1_op = tf.reduce_sum(
        #                     tf.cast(tf.equal(labels, axis=1), tf.argmax(labels_placeholder, axis=1)), tf.int32), name="Right_Count_Top1")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_dir", type=str, default="./tensorboard_restore", help="Model folder to export")
    parser.add_argument("--output_node_names", type=str, default="labels",
